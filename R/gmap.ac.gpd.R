...
#'
#' @description The gmap.ac.gpd function performs genomic mediation analysis
...
#'   permutation times.} \code{calculate Empirical P-values using GPD
#'   fitting}{The use of a fixed number of permutations to calculate empirical
...
#'   is a PC.} \item{sel.conf.ind}{An indicator matrix with dimension of the
#'   number of trios by the number of covariates in \code{cov.pool} or
#'   \code{pc.matrix}if the principal components (PCs) based on expression data
...
#'
gmap.ac.gpd <- function(snp.dat, fea.dat, known.conf, trios.idx, cl = NULL, cov.pool = NULL, pc.num = 30,
						Minperm = 100, Maxperm = 10000, gpd.perm = 0.01, fdr = 0.05, fdr_filter = 0.1){
	...
	if(!is.null(cl)){
		...
		known_sel_pool_output <- parLapply(cl, 1:num_trio, getp.func, triomatrix = triomatrix, confounders = confounders,
		                                   pool_cov = pool_cov, est_conf_pool_idx = est_conf_pool_idx,
		                                   Minperm = Minperm, Maxperm = Maxperm, use.gpd=TRUE, gpd.perm = gpd.perm, use.PC = use.PC)
	}else{
		known_output <- lapply(1:num_trio, getp.func, triomatrix = triomatrix, confounders = confounders,
		                       Minperm = Minperm, Maxperm = Maxperm, use.gpd=TRUE, gpd.perm = gpd.perm)
		known_sel_pool_output <- lapply(1:num_trio, getp.func, triomatrix = triomatrix, confounders = confounders,
										pool_cov = pool_cov, est_conf_pool_idx = est_conf_pool_idx,
										Minperm = Minperm, Maxperm = Maxperm, use.gpd=TRUE, gpd.perm = gpd.perm, use.PC = use.PC)
	}
